...
public class CSV {
    ...
    public convenience init?(contentsOfFile file: String, error: NSErrorPointer) {
        ...
        self.init(contentsOfFile: file, delimiter: delimiter, encoding: NSUTF8StringEncoding, error: error)
    }
    
    public convenience init?(contentsOfURL file: String, encoding: UInt, error: NSErrorPointer) {
        let delimiter = NSCharacterSet(charactersInString: "|")
        self.init(contentsOfFile: file, delimiter: delimiter, encoding: encoding, error: error)
    }
